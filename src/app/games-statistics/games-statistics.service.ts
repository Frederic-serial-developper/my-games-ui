...
export class CollectionStatisticsService {
    ...
    getCollectionStatistics(): Observable<CollectionStatistics> {
        return this.http.get("http://localhost:8080/my-games-services/bgg/collection/fredericdib/stats?includeExpansions=false").map((response: Response) => {
           return <CollectionStatistics>response.json();
        }).catch(this.handleError);
    }
